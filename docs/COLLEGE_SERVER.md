...
IPFS (Pinata) and Polygon anchoring run from **ingest** when `PINATA_JWT`, `DATACHAIN_CONTRACT_ADDRESS`, and the owner key are set in the backend `.env`. Without those, minutes stay in `temp/` and retry. Do not put the Amoy private key on the college VM unless you intend ingest to spend testnet POL there. Amoy deploy remains `npm run deploy:amoy` from `contracts/` on a trusted machine.
