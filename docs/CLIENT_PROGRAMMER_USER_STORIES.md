...
| CP-C.P7 | As the system, I want to delete temp files only after successful processing, and keep files that failed or are awaiting retry. | Implemented — worker deletes only after processor success; ingest success is Pinata + chain + DB |
| CP-C.P8 | As the system, I want to detect unexpected FFmpeg / stream stop, restart with a capped attempt count, mark the camera offline after repeated failures, and log restart attempts. | Implemented — ingest restarts FFmpeg (default cap 10); logs include camera id; `ingest_offline_at` after cap; dashboard `status` offline |
...
- **CP-C.P2–P8** attach FFmpeg to each active camera’s `stream_url` and split it into 1-minute `.mp4` files named `{camera_id}_{start}Z.mp4` under `backend/temp/<camera-id>/`. Each closed file is integrity-checked (complete MP4, video stream, SHA-256) before later processing; failures stay in `temp/` and are not handed on. Passing files **remain staged under `temp/` until Pinata upload, chain anchor, and DB save succeed**. Temp files are **deleted only after that processing succeeds**; failures stay on disk and are retried. If FFmpeg stops unexpectedly, ingest **restarts** it (default cap 10, logged with camera id) and **marks the camera offline** after the cap (`ingest_offline_at`; apply Alembic `20260824_000003`). Run from `backend/`: `python scripts/ingest_camera.py --camera-id UUID` or `--all`. Requires `PINATA_JWT`, `DATACHAIN_CONTRACT_ADDRESS`, and the owner key. Parse start/end with `app.services.segment_identity.parse_segment_path`.
...
| ID | Story | Status |
| -- | ----- | ------ |
| CP-D.C1 | As a user, I want each recorded minute to be stored and anchored so I can later prove integrity (experienced via verification UI). | Implemented — camera recordings Verify compares API metadata to on-chain `getSegment` (E3) |
...
| CP-E.C6 | As a user, I want to download the selected recording as a usable local file. | Implemented — Download concatenates overlapping minutes via FFmpeg into one `.mp4` (`GET /cameras/{id}/recordings/download`) |
...
| CP-E.C12 | As a user, I want to open a segment’s transaction hash on Polygonscan from the verification result. | Implemented — “View on Polygonscan” when `tx_hash` is present |
...
| CP-E.P3 | As the system, I want to calculate which one-minute segments are **expected** for a selected range, so gaps can be detected. | Implemented — UTC minute slots overlapping the applied window (`expectedMinuteStarts`) |
| CP-E.P4 | As the system, I want to check whether each expected segment is available in storage, distinguishing missing metadata from an unretrievable file. | Implemented — no DB row for a slot is Missing; RPC errors are Failed to verify (not tampered). The browser does not fetch the IPFS blob (CORS would look like failure); Watch covers playback |
