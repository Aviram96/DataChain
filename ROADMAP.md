...
**Progress note**: Contract store/read + tests landed 2026-09-14 (`Datachain.sol`, `contracts/test/Datachain.ts`) — **US-6.4** in progress locally. **US-6.5** Amoy deploy path (`scripts/deploy.ts`); **US-6.6** live Amoy address after the maintainer runs it. **US-6.7 / US-6.8 / US-6.9** Web3.py `chain_anchor.py` (timeout/gas retry; ingest not wired). **US-6.1 / US-6.2** Pinata upload helper — ingest not wired. **US-6.10** schema ready; pipeline does not persist yet. See **CP-D.P3** / **CP-D.P4** Implemented; **CP-D.P1** / **CP-D.P2** / **CP-D.P6** Partial / **CP-D.P8** Implemented.
...
| D | IPFS storage and blockchain anchoring | In progress — CP-D.P3 / CP-D.P4 / CP-D.P8 Implemented; CP-D.P1 / CP-D.P2 / CP-D.P6 Partial |
