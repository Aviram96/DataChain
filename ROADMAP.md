...
| US-2.3 | As the system, I need each video record linked to a camera with segment start and end timestamps.         | Done — `backend/app/models/video_record.py` (`camera_id`, `started_at`, `ended_at`) |
| US-2.4 | As the system, I need each video record to store an IPFS CID and blockchain transaction hash.            | Done — `ipfs_cid`; `tx_hash` nullable until anchored; `segment_hash` added in Alembic `20260914_000004` |
...
**Progress note**: A global **My Videos** page (US-7.1) is deferred; footage stays camera-scoped on `/cameras/[id]`. E1 lists/filters/paginates that camera’s minutes (`GET /cameras/{id}/recordings`). E2 streams a selected minute from `{NEXT_PUBLIC_IPFS_GATEWAY}/ipfs/{cid}` (default `https://gateway.pinata.cloud`; see `frontend/.env.example`).
...
| E | Video management and verification | In progress — E1 search/list; E2 Watch; E3 full-range verify; E4 partial-range verify (CP-E.C8 / P9); CP-E.C2 Partial (download later) |
